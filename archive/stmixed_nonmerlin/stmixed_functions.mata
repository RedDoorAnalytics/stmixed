...
{
//log(S.detcholVCVs :* (*SS).jlnodes  * S.adaptbaseGHweights) :+ S.c[i] :-708

	//like_j = (*SS).detcholVCVs :* (*SS).jlnodes * (*SS).adaptbaseGHweights :* exp((*SS).c)
	//like_j = exp(quadrowsum((*SS).jlnodes,1) :+ (*SS).c)
	(*SS).jlnodes
	loglf 	= log(quadrowsum((*SS).jlnodes,1)) :+ (*SS).c
	loglf,exp(loglf),(*SS).c
	exit(111)
	lf = exp(log(quadrowsum((*SS).jlnodes,1)) :+ (*SS).c)
	log(quadrowsum((*SS).jlnodes,1))
	Nres = (*SS).Nres
	
	for(i=1;i<=(*SS).N;i++) {
		nodes_i = asarray((*SS).GHNarray,i)	
		
		test1 = exp((*SS).jlnodes[i,] :+ (*SS).c[i]):/lf[i]
		
		//test1 = (*SS).detcholVCVs[i,] :* (*SS).jlnodes[i,] :/like_j[i,] 
		//newblups =  test1 * (nodes_i :* (*SS).adaptbaseGHweights')'
		newblups =  test1 * (nodes_i)'
		newblups
		//vcv_new = (test1 * (stmixed_outerprod_by_col(nodes_i') :* (*SS).adaptbaseGHweights)) :- rowshape((newblups')*newblups,1)
		vcv_new = (test1 * (stmixed_outerprod_by_col(nodes_i'))) :- rowshape((newblups')*newblups,1)
...
`NM' stmixed_logdmvnorm(`NM' X, `NM' V)
{
	`NM' invsigma, Vvecs, Vvals, q2, denom
