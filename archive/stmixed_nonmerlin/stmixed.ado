...
program Estimate, eclass
	...
	// model opts
		...
		local modelopts `"`modelopts' BHazard(varname)"'
	...
	//Max opts
		...
		local modelopts `"`modelopts' LFScale(real 0)"'	
	...
	//-> parse model options
        ...
        syntax [ , `modelopts' *]
			...
			if `aft' & "`bhazard'"!="" {
				di as error "bhazard() not allowed with AFT models"
				exit 198
			}
	...
	// Starting values
		...
		if "`vcvinitmatrix'"=="" {
			...
			mat `initmat' = `initmat',J(1,`nvarparams',-1.5)
